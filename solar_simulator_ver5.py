...
# =========================================================
st.set_page_config(
    page_title="AI 가변형 태양광 펜스 시뮬레이션",
    layout="wide"
)

st.title("🌞 AI 가변형 태양광 펜스 시뮬레이션")
...
# --- 전력 단가 선택 ---
_PRICE_OPTS = {
    "일반용 전력(갑)I 저압 — 130원/kWh (연평균 근사)": 130.0,
    "일반용 전력(갑)I 저압 — 여름철 132.4원/kWh":     132.4,
    "가로등(갑) 정액등 — 참고: 47.2원/W·월 (단위 상이)": None,
    "직접 입력": None,
}
price_selection = st.sidebar.selectbox(
    "전력 단가 기준 선택",
    list(_PRICE_OPTS.keys()),
    index=0
)
if _PRICE_OPTS[price_selection] is not None:
    electricity_price = _PRICE_OPTS[price_selection]
    st.sidebar.caption(f"적용 단가: **{electricity_price:.1f}원/kWh**")
elif price_selection == "가로등(갑) 정액등 — 참고: 47.2원/W·월 (단위 상이)":
    st.sidebar.warning(
        "가로등(갑) 정액등 단가는 W당 47.2원/월 기준으로, "
        "kWh 단가와 단위가 달라 직접 비교가 불가합니다. "
        "아래에 환산값을 직접 입력하세요."
    )
    electricity_price = st.sidebar.number_input(
        "직접 입력 (원/kWh)", min_value=1.0, max_value=500.0, value=47.0, step=1.0
    )
else:
    electricity_price = st.sidebar.number_input(
        "직접 입력 (원/kWh)", min_value=1.0, max_value=500.0, value=130.0, step=1.0
    )

with st.sidebar.expander("전기요금 참고표 (2025년 4월 1일 시행)"):
    st.markdown("""
| 용도 | 구분 | 전력량요금 (원/kWh) | 비고 |
|---|---|---|---|
| 일반용(갑)I 저압 | 여름철(6~8월) | 132.4 | 계약전력 300kW 미만 |
| 일반용(갑)I 저압 | 봄·가을철 | 91.9 | |
| 일반용(갑)I 저압 | 겨울철(11~2월) | 119.0 | |
| 가로등(갑) 정액등 | — | 47.2원/**W·월** | kWh 환산 불가 |

출처: 한국전력공사, 「2025년 4월 1일 시행 전기요금표(종합)」
    """)

# --- 전력 배출계수 선택 ---
_EF_OPTS = {
    "소비단 CO₂eq. — 0.4541 kgCO₂eq/kWh (2024년 승인, 2020~2022 평균)": 0.4541,
    "발전단 CO₂eq. — 0.4172 kgCO₂eq/kWh (2024년 승인, 2020~2022 평균)": 0.4172,
    "소비단 CO₂eq. — 0.4173 kgCO₂eq/kWh (2023년 최신)":                  0.4173,
    "직접 입력": None,
}
ef_selection = st.sidebar.selectbox(
    "탄소 배출 계수 기준 선택",
    list(_EF_OPTS.keys()),
    index=0
)
if _EF_OPTS[ef_selection] is not None:
    carbon_factor = _EF_OPTS[ef_selection]
    st.sidebar.caption(f"적용 계수: **{carbon_factor} kgCO₂eq/kWh**")
else:
    carbon_factor = st.sidebar.number_input(
        "직접 입력 (kgCO₂eq/kWh)", min_value=0.1, max_value=1.0,
        value=0.4541, step=0.001, format="%.4f"
    )

with st.sidebar.expander("전력 배출계수 참고표 (2024년 승인)"):
    st.markdown("""
| 부문 | 계수 | 계수값 | 단위 |
|---|---|---|---|
| 발전단 | CO₂eq. | **0.4172** | t CO₂eq/MWh |
| 발전단 | CO₂ | 0.4150 | t CO₂/MWh |
| 발전단 | CH₄ | 0.0044 | kg CH₄/MWh |
| 발전단 | N₂O | 0.0077 | kg N₂O/MWh |
| 소비단 | CO₂eq. | **0.4541** | t CO₂eq/MWh |
| 소비단 | CO₂ | 0.4517 | t CO₂/MWh |
| 소비단 | CH₄ | 0.0048 | kg CH₄/MWh |
| 소비단 | N₂O | 0.0084 | kg N₂O/MWh |

※ 2024년 승인 = 2020~2022년 국가통계 기반 (06 IPCC 지침)
출처: 환경부 온실가스종합정보센터,
「2024년 승인 국가 온실가스 배출계수_전력배출계수」, 2025.03.31
    """)
...
# [개선 1] 태양 방위각 계산 (북=0°, 동=90°, 남=180°, 서=270°)
def solar_azimuth(latitude_deg, declination_deg, hour):
    ...
    return np.degrees(np.arctan2(sin_az, cos_az)) % 360
...
def incidence_angle_factor(panel_angle_deg, solar_alt_deg, solar_az_deg, face_az_deg):
    ...
    sx = np.sin(saz)*np.cos(alt); sy = np.cos(saz)*np.cos(alt); sz = np.sin(alt)
    nx = np.sin(pan)*np.sin(faz); ny = np.sin(pan)*np.cos(faz); nz = np.cos(pan)
    return max(0.0, sx*nx + sy*ny + sz*nz)


def calculate_power(panel_angle_deg, solar_alt_deg, solar_az_deg, weather_condition):
    factor = incidence_angle_factor(panel_angle_deg, solar_alt_deg, solar_az_deg, panel_face_az)
    return solar_irradiance * panel_area * panel_efficiency * factor * weather_factor(weather_condition)


def ai_optimal_angle(solar_alt_deg, solar_az_deg):
    """울타리 방향을 반영한 발전량 최대화 패널 각도"""
    if solar_alt_deg <= 0:
        return 0.0
    alt     = np.radians(solar_alt_deg)
    ...
    if B < 1e-9 and C <= 0:
        return 0.0
    return float(np.clip(np.degrees(np.arctan2(C, B)), 0, 90))
...
def calculate_motor_energy(angle_change_deg):
    ...
    return torque * np.radians(abs(angle_change_deg)) / motor_efficiency / 3600
...
current_ai_ang = ai_optimal_angle(current_alt, current_az)

current_fixed_pw    = calculate_power(fixed_panel_angle, current_alt, current_az, weather)
current_tracking_pw = calculate_power(current_ai_ang,   current_alt, current_az, weather)

st.subheader("현재 시간 기준 결과")
c1, c2, c3, c4, c5 = st.columns(5)
with c1: st.metric("태양 고도각",      f"{current_alt:.2f}°")
with c2: st.metric("태양 방위각",      f"{current_az:.1f}°")
with c3: st.metric("AI 최적 패널 각도", f"{current_ai_ang:.2f}°")
with c4: st.metric("고정형 발전량",    f"{current_fixed_pw:.2f} W")
with c5: st.metric("AI 최적각 발전량", f"{current_tracking_pw:.2f} W",
                   f"{current_tracking_pw - current_fixed_pw:.2f} W")
...
last_step_time  = 6.0 - tracking_interval_hr  # 첫 스텝에서 즉시 최적화
...
for t in times:
    ...
    sne = sgp * time_step_hour - sme
    snp = sne / time_step_hour

    solar_altitudes.append(alt);     solar_azimuths_list.append(az)
...
df = pd.DataFrame({
    "Time": times,
    "Solar Altitude": solar_altitudes,  "Solar Azimuth":  solar_azimuths_list,
    "AI Optimal Angle": optimal_angles,
    "Fixed Power": fixed_powers,        "Fixed Energy": fixed_energies,
    "Continuous Angle": cont_angles,    "Continuous Gross Power": cont_gross_powers,
    "Continuous Motor Energy": cont_motor_energies,
    "Continuous Net Power": cont_net_powers,
    "Continuous Net Energy": cont_net_energies, "Continuous Gain": cont_gains,
    "Step Angle": step_angles,          "Step Gross Power": step_gross_powers,
    "Step Motor Energy": step_motor_energies,
    "Step Net Power": step_net_powers,
    "Step Net Energy": step_net_energies, "Step Gain": step_gains,
})
...
with sc1: st.metric("고정형",          f"{fixed_scaled:.1f} Wh", f"{fixed_scaled/1000:.3f} kWh")
with sc2: st.metric("연속 추적형",     f"{cont_scaled:.1f} Wh",  f"{cont_scaled/1000:.3f} kWh")
with sc3: st.metric(f"간헐 추적형 ({tracking_interval_min}분)",
                    f"{step_scaled:.1f} Wh", f"{step_scaled/1000:.3f} kWh")
...
def grid_metrics(generated_wh):
    eff   = generated_wh * (1 - local_wire_loss_rate)
    needed = eff / (1 - transmission_loss_rate) if transmission_loss_rate < 1 else 0
    return eff, needed, needed - generated_wh


fix_eff,  fix_grid,  fix_ls   = grid_metrics(fixed_scaled)
cont_eff, cont_grid, cont_ls  = grid_metrics(cont_scaled)
step_eff, step_grid, step_ls  = grid_metrics(step_scaled)

tl1, tl2, tl3 = st.columns(3)
for col, label, eff, grid, ls in [
    (tl1, "고정형",                              fix_eff,  fix_grid,  fix_ls),
    (tl2, "연속 추적형",                          cont_eff, cont_grid, cont_ls),
    (tl3, f"간헐 추적형 ({tracking_interval_min}분)", step_eff, step_grid, step_ls),
]:
    with col:
        st.markdown(f"**{label}**")
...
st.caption(
    f"전력 단가: {electricity_price:.1f}원/kWh  |  "
    f"탄소 배출 계수: {carbon_factor:.4f} kgCO₂eq/kWh  |  "
...
with ec1:
    ...
    st.metric("연간 CO₂ 절감량",   f"{cont_gain_annual * carbon_factor:.2f} kgCO₂eq")
with ec2:
    ...
    st.metric("연간 CO₂ 절감량",   f"{step_gain_annual * carbon_factor:.2f} kgCO₂eq")
...
for iv_min in range(10, 130, 10):
    ...
    for t in times:
        ...
        if (t - lst) >= iv_hr - 1e-9:
            ...
            psa = opt; csa = opt; lst = t
        else:
            m_e = 0.0
        total += calculate_power(csa, alt, az, weather) * time_step_hour - m_e
...
fig_iv.update_layout(
    xaxis_title="추적 간격", yaxis_title="순 발전량 (Wh)", height=380,
    annotations=[dict(
        text=(f"최적: {best_iv}분 ({interval_results[best_iv]:.1f} Wh)"
              f"  |  현재 설정: {tracking_interval_min}분"),
...
- 연속 추적형 기준 추가 절감: **{extra_cont:.1f} Wh/일**

**연간 경제성 (연속 추적형, 고정형 대비)**
- 추가 발전량: **{cont_gain_annual:.2f} kWh/년**
- 전기요금 절감: **{cont_gain_annual * electricity_price:,.0f}원/년** (단가 {electricity_price:.1f}원/kWh 기준)
- CO₂ 절감: **{cont_gain_annual * carbon_factor:.2f} kgCO₂eq/년** (계수 {carbon_factor:.4f} 기준)
""")
...
# =========================================================
st.markdown("---")
st.subheader("공공데이터 출처 및 참고 사항")

st.markdown("""
본 시뮬레이터에 사용된 주요 수치는 아래 공공기관 공식 자료를 기반으로 합니다.
사용자가 직접 값을 수정하는 경우에도 동일한 출처에서 최신값을 확인하시기 바랍니다.
""")

with st.expander("① 국가 고유 전력 배출계수 출처", expanded=True):
    st.markdown("""
**발행 기관**: 환경부 온실가스종합정보센터 (GIR)
**문서명**: 「2024년 승인 국가 온실가스 배출계수_전력배출계수」
**발행일**: 2025년 3월 31일
**산정 기준**: 2020~2022년 국가통계 (IPCC 2006 지침 적용)

| 부문 | 계수 종류 | 적용값 | 단위 |
|---|---|---|---|
| **소비단** | CO₂eq. | **0.4541** | t CO₂eq/MWh (= kg CO₂eq/kWh) |
| **발전단** | CO₂eq. | **0.4172** | t CO₂eq/MWh (= kg CO₂eq/kWh) |
| 소비단 | CO₂ only | 0.4517 | t CO₂/MWh |
| 발전단 | CO₂ only | 0.4150 | t CO₂/MWh |

> **소비단 계수**: 송배전 손실을 포함한 최종 소비 기준 — 수요자 측 탄소 산정에 사용
> **발전단 계수**: 발전소 출구 기준 — 공급자 측 탄소 산정에 사용

🔗 **문서 직접 다운로드**: https://www.gir.go.kr/home/file/readDownloadFile.do?fileId=7759&fileSeq=1
🔗 **게시판 페이지**: https://www.gir.go.kr/home/board/read.do?menuId=36&boardId=82&boardMasterId=2
    """)

with st.expander("② 연간 전력 계통 손실률 출처", expanded=True):
    st.markdown("""
**발행 기관**: 한국전력거래소 (KPX) — 전력통계정보시스템 (EPSIS)
**데이터명**: 전력손실량 통계 (송변전손실 / 배전손실 / 송배전손실)
**단위**: MWh 및 % / **조회 범위**: 1961년~현재
**갱신 주기**: 매년 7월경 (확정치 6월 공표, 이전은 잠정치)

| 항목 | 설명 |
|---|---|
| 송변전손실 | 발전소 → 변전소 구간 손실 |
| 배전손실 | 변전소 → 최종 소비자 구간 손실 |
| **송배전손실** | **위 둘을 합산한 총 손실 (코드 적용값)** |

> 본 시뮬레이터 기본값 **3.5%**는 근사값입니다.
> 정확한 최신 연도값은 아래 링크에서 Excel/CSV로 직접 다운로드하여 확인하세요.

🔗 **전력손실량 통계 페이지**: https://epsis.kpx.or.kr/epsisnew/selectEksaPlrChart.do?menuId=060700
    """)

with st.expander("③ 전기요금 단가 출처", expanded=True):
    st.markdown("""
**발행 기관**: 한국전력공사 (KEPCO)
**문서명**: 「2025년 4월 1일 시행 전기요금표(종합)」
**시행일**: 2025년 4월 1일

| 용도 | 구분 | 여름철 (6~8월) | 봄·가을철 (3~5, 9~10월) | 겨울철 (11~2월) | 단위 |
|---|---|---|---|---|---|
| 일반용(갑)I | 저압 (300kW 미만) | **132.4** | 91.9 | 119.0 | 원/kWh |
| 일반용(갑)I | 고압A 선택Ⅰ | 142.6 | 98.6 | 130.3 | 원/kWh |
| 가로등(갑) | 정액등 | — | — | — | **47.2원/W·월** |

> ⚠️ 가로등(갑) 정액등 단가 **47.2원/W·월**은 계약전력(W) 기준 월정액 요금으로,
> kWh 단가와 단위가 달라 직접 비교 불가 — 시뮬레이터에서 직접 입력 시 환산 필요
> 참고 환산식: 월 소비 kWh = 패널 W × 일 평균 발전 시간 × 30일

🔗 **전기요금표 전체 목록**: https://online.kepco.co.kr/PRM004D00
🔗 **전기공급약관 별표1 (법적 기준)**: https://cyber.kepco.co.kr/ckepco/front/jsp/CY/D/C/CYDCHP00401.jsp
    """)

with st.expander("④ 태양 위치 계산 공식 출처"):
    st.markdown("""
| 공식 | 출처 |
|---|---|
| 태양 적위각 δ = 23.45° × sin(360(284+N)/365) | Duffie, J.A. & Beckman, W.A., *Solar Engineering of Thermal Processes*, 4th ed., Wiley, 2013 |
| 태양 고도각 sin(α) = sin(φ)sin(δ) + cos(φ)cos(δ)cos(H) | 위 동일, Eq. 1.6.5 |
| 태양 방위각 atan2(sin_Az, cos_Az) | 위 동일, Eq. 1.6.6 |
| 패널 입사각 cos(θ) = **n̂** · **ŝ** (3D 벡터 내적) | 위 동일, Eq. 1.8 |
| 모터 에너지 E = τθ/η, τ = mgr | 고전 강체 회전 역학 |
    """)

st.caption(
    "※ 본 시뮬레이터는 교육·연구 목적의 수치 검증 도구입니다. "
    "실제 설계 및 경제성 분석에는 현장 측정값과 최신 공시 자료를 반드시 사용하십시오."
)
